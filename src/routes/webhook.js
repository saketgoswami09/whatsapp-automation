...
const { verifyWebhookSignature, parseWebhookPayload, sendText, markMessageRead } = require('../utils/whatsapp');
const { getOrCreateUser, getOrCreateConversation, saveMessage } = require('../services/conversationService');
...
router.get('/', (req, res) => {
  ...
  return res.status(403).json({ status: 'fail', message: 'Webhook verification failed' });
});

/**
 * POST /webhook — Receive incoming messages from Meta
 */
router.post('/', webhookLimiter, express.json({ verify: verifyRawBody }), async (req, res) => {
  // Always reply 200 immediately to Meta to prevent retries
  res.status(200).send('EVENT_RECEIVED');

  try {
    const signature = req.headers['x-hub-signature-256'];
    if (env.NODE_ENV === 'production' && !verifyWebhookSignature(req.rawBody, signature)) {
